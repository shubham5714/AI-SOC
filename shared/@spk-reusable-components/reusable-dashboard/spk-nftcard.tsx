...
import { HotBidsType } from '@/shared/data/dashboards/nftdata'
...
const SpkNftCard: React.FC<SpkNftCardProps> = ({ card, customCardClass, cardBodyClass }) => {
    return (
        <Fragment>
            <Card className={`custom-card ${customCardClass}`}>
                <div className="mb-0 text-fixed-white bg-info nft-auction-time">
                    {card.time}
                </div>
                <div className="position-relative custom-width">
                    <Image fill src={card.image} className="card-img-top nft-img1" alt="..." />
                    <SpkBadge variant="" Customclass="nft-like-badge text-fixed-white"><i className="ri-heart-fill me-1 text-danger align-middle d-inline-block"></i>{card.likes}</SpkBadge>
                </div>
                <Card.Body className={cardBodyClass}>
                    ...
                    <div className="d-flex mb-3 align-items-center flex-wrap gap-2">
                        <div className="lh-1">
                            <span className="avatar avatar-rounded avatar-xs custom-width">
                                <Image fill src={card.src} alt="" />
                            </span>
                        </div>
                        <div className="flex-fill">
                            <p className="mb-0 fs-12 fw-medium">{card.name} </p>
                            <p className="fs-11 op-8 mb-0 lh-1">- {card.handle}</p>
