...
const generateDayWiseTimeSeries = function (baseval: number, count: number, yrange: { min: number; max: number; }) {
	let i = 0;
	const series: number[] = [];
	while (i < count) {
		const x = baseval;
...
export const Areastackoptions = {
	chart: {
		...
		stacked: true,
		events: {
			selection: function (_chart: number, e: { xaxis: { min: string | number | Date; }; }) {
...
export const Areatimeoptions = {
	...
	xaxis: {
		...
		labels: {
			rotate: -15,
			rotateAlways: true,
			formatter: function (val: string, timestamp: string | number | Date) {
